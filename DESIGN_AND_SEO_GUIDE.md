...
**Hand-picked cases test the path you already understand.** They cluster around defaults, so any rule that only fires at the edges of the input space survives them. For every calculator, after the targeted assertions pass, run a randomised sweep across the full input range and diff every output field. Where the reference exposes internals (emitted JS variables, slider `min`/`max` attributes), read the rule off them directly instead of inferring it from displayed values — the Macro protein floor was settled in one probe by reading the protein slider's `min`, after several wrong inferences from the rendered numbers.
...
- **A raw-HTML audit is necessary but not sufficient — render their page too.** Step 1 says fetch the live page, and fetching the *source* is the right way to catch inputs that the rendered view hides (the inflation page's month dropdowns are empty `<select>`s in source, populated by an `updateMonths()` script that also encodes their data cutoff — the markdown extraction lost all of it). But the reverse failure exists as well: their heavier pages build **result tables and charts in JavaScript**, so those outputs appear nowhere in the fetched HTML. The Inflation Calculator shipped a first build missing their full year-by-month inflation rate table (113 rows x 14 columns under the historical chart) for exactly this reason; it was caught only when the owner sent screenshots. Do both — read the source for the input set, and render the page (headless screenshot, or ask the owner for one) for the output set — before declaring the step-2 field map and step-5 result list complete.
...
- **Defaults**: where a calculator needs a default currency, unit, or region-specific assumption, default to USD / US units / US conventions. The only exception is a tool that is *about one named country's system* and could not sensibly show anything else (a UK-mortgage calculator, a Canadian-mortgage calculator). **A VAT calculator is NOT such a tool** — an earlier version of this line used it as the example and that was wrong. It was read as licence to build the page UK-first, in GBP, with a UK country preset preselected; the owner rejected that on Aug 7, 2026 and the page was rebuilt USD-first. VAT is a *concept* foreign to the US, not a page about one country: a US visitor lands there because they sell into VAT countries, buy from them, or travel, so the page defaults to USD and leads with the US angle while still offering every country's rate one click away. **Standing rule: a US-first default is never the thing to deviate from without asking the owner first.** If a genuine case for a non-US default appears, raise it before building, do not decide it inside a build.
